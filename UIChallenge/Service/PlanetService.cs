...
namespace UIChallenge.Service
{
        ...
        private static List<Planets> planets = new()
    {
        ...
        new()
        {
            ...
            "Venus has a thick, toxic atmosphere primarily composed of carbon dioxide, with clouds of sulfuric acid, which creates a runaway greenhouse effect, making it the hottest planet in our solar system. Surface temperatures can soar to over 900 degrees Fahrenheit (475 degrees Celsius), which is hotter than the surface of Mercury, even though Venus is farther from the Sun.\r\n\r\nThe planet's surface is shrouded in a dense layer of clouds, making it difficult to observe its features from space. However, radar imaging has revealed a stark and rocky landscape with numerous volcanoes, impact craters, and vast highland regions.\r\n\r\nVenus rotates very slowly and in the opposite direction to most other planets, which means its day is longer than its year. A day on Venus (a full rotation) takes about 243 Earth days, while its orbit around the Sun takes roughly 225 Earth days. Venus is often called Earth's “sister” planet. As similar as it is in some ways, however, it is also very different in others.",
        ...
        public static List<Planets> GetAllPlanets()
            => planets;

        public static Planets GetPlanet(string planetName)
        ...
        public static IEnumerable<Planets> GetSearchedPlanet(string planetName)
        {
            var searchedValue = planets.Where(x => x.Name == planetName);
            if(planetName == "")
            {
                return planets;
            }
